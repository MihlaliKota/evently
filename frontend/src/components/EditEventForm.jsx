...
const EditEventForm = ({ open, onClose, onEventUpdated, eventData }) => {
    // Form state
    ...
    // Reset and initialize form when event data changes
    useEffect(() => {
        if (open && eventData) {
            setFormData({
                name: eventData.name || '',
            ...
            // Set original image URL and preview if available
            if (eventData.image_path) {
                setOriginalImageUrl(eventData.image_path);
    ...
    // Form submission for updating event
    const handleSubmit = useCallback(async (e) => {
        e.preventDefault();
        
        if (!validateForm() || !eventData || !eventData.event_id) {
            return;
        }
        ...
        try {
            ...
            const data = await api.events.updateEventWithImage(eventData.event_id, formDataObj);
    ...
    }, [formData, selectedImage, validateForm, onEventUpdated, onClose, eventData]);
            ...
            >
                ...
                <DialogContent dividers>
                    ...
                    <Typography variant="body1">
                        You need administrator privileges to create and manage events. Please contact an administrator if you need to edit an event.
                    </Typography>
                </DialogContent>
    ...
    return (
        ...
        >
            <DialogTitle>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Typography variant="h6">Edit Event: {eventData?.name}</Typography>
                    <Button
                        color="inherit"
                        onClick={handleClose}
                        startIcon={<Close />}
